# Operator local Helm chart deployment test

Installs `charts/pii-shield-operator` from the current checkout, then runs the shared access-log replay workload.

```bash
tests/deployments/operator-helm-local/run.sh
```

Sanitized logs are written to `/tmp/pii-shield-sanitized/operator-helm-local`.
...
tests/deployments/operator-helm-local/chart.sh status
...
| --- | --- | --- |
| `template` | Renders the local `charts/pii-shield-operator` chart to YAML without touching the cluster. | Check generated manifests, CRDs, webhook, RBAC, image tags, and values before install. |
| `lint` | Runs Helm chart validation. | Catch chart syntax/schema issues before installing. |
| `upgrade` | Runs `helm upgrade --install` for release `pii-shield-operator` in namespace `operator-system`. | Install the local chart or apply chart/value changes. |
...
```bash
tests/deployments/operator-helm-local/chart.sh template > /tmp/pii-shield-operator.yaml
...
REVISION=1 tests/deployments/operator-helm-local/chart.sh rollback
```
...
```bash
tests/deployments/operator-helm-local/cleanup.sh
```
